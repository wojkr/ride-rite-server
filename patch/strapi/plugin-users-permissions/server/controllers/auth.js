...
module.exports = {
  async callback(ctx) {
    ...
    if (provider === "local") {
      ...
      //changes start (2/2)
      const userPopulated = await getService("user").fetch(
        user.id,
        sanitizedQuery
      );

      const sanitizedUser = await sanitizeOutput(userPopulated, ctx);
      return ctx.send({
        jwt: getService("jwt").issue({ id: userPopulated.id }),
        user: { ...sanitizedUser, orders: userPopulated.orders },
      });
      //changes end (2/2)
    }
  },
